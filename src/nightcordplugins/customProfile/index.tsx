...
function CheckIcon() {
    return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m20 6-11 11-5-5" /></svg>;
}

function getConnectionInitials(type: string) {
    const platform = CONNECTION_PLATFORMS.find(p => p.type === type);
...
function FakeConnectionEditor({ connections, onChange }: { connections: FakeConnection[]; onChange: (connections: FakeConnection[]) => void; }) {
    ...
    function updateConnection(id: string, updates: Partial<FakeConnection>) {
        onChange(connections.map(c => c.id === id ? { ...c, ...updates } : c));
    }
    ...
    return (
        <div className="cp-field">
            <SectionLabel>{t("Fake Connections")}</SectionLabel>
            <div className="cp-connection-shell">
                <div className="cp-fake-connections">
                    {connections.length === 0 && (
                        <div className="cp-connection-empty">
                            <div className="cp-connection-avatar cp-connection-avatar-empty">+</div>
                            <div>
                                <strong>{t("No fake connections yet")}</strong>
                                <span>{t("Add one below to show it on your profile.")}</span>
                            </div>
                        </div>
                    )}
                    {connections.map(conn => {
                        const platform = CONNECTION_PLATFORMS.find(p => p.type === conn.type);
                        const isEditing = editingId === conn.id;
                        return (
                            <div key={conn.id} className="cp-connection-item">
                                {isEditing ? (
                                    <div className="cp-connection-edit">
                                        <div className="cp-connection-edit-grid">
                                            <select
                                                className="cp-input"
                                                value={conn.type}
                                            ...
                                            <input
                                                className="cp-input"
                                                placeholder={t("Likes (optional)")}
                                                value={conn.likes || ""}
                                                onChange={e => updateConnection(conn.id, { likes: e.target.value })}
                                            />
                                        </div>
                                        <div className="cp-connection-edit-actions">
                                            <label className="cp-checkbox-label">
                                                <input
                                                    type="checkbox"
                                                    checked={conn.verified}
                                                    onChange={e => updateConnection(conn.id, { verified: e.target.checked })}
                                                />
                                                <span>{t("Verified")}</span>
                                            </label>
                                            <button className="cp-btn cp-btn-primary cp-btn-small" onClick={() => setEditingId(null)}><CheckIcon /><span>{t("Done")}</span></button>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="cp-connection-display">
                                        <div className="cp-connection-avatar">{getConnectionInitials(conn.type)}</div>
                                        <div className="cp-connection-main">
                                            <div className="cp-connection-title">
                                                <span className="cp-connection-name">{conn.name}</span>
                                                {conn.verified && <span className="cp-connection-verified" title={t("Verified")}><CheckIcon /></span>}
                                            </div>
                                        ...
                                        <div className="cp-connection-actions">
                                            <button className="cp-btn cp-btn-ghost cp-btn-small" onClick={() => setEditingId(conn.id)}><EditIcon size={14} /><span>{t("Edit")}</span></button>
                                            <button className="cp-btn cp-btn-danger cp-btn-icon" onClick={() => deleteConnection(conn.id)} title={t("Delete")}><TrashIcon /></button>
                                        </div>
                                    </div>
                                )}
                            </div>
                ...
                <div className="cp-connection-add">
                    <div className="cp-connection-add-grid">
                        <select
                            className="cp-input"
                        ...
                        <input
                            className="cp-input"
                            placeholder={t("Followers (optional)")}
                            value={newConnection.followers}
                            onChange={e => setNewConnection({ ...newConnection, followers: e.target.value })}
                        />
                        <input
                            className="cp-input"
                            placeholder={t("Likes (optional)")}
                            value={newConnection.likes}
                            onChange={e => setNewConnection({ ...newConnection, likes: e.target.value })}
                        />
                    </div>
                    <div className="cp-connection-add-actions">
                        <label className="cp-checkbox-label">
                            <input
                                type="checkbox"
                                checked={newConnection.verified}
                                onChange={e => setNewConnection({ ...newConnection, verified: e.target.checked })}
                            />
                            <span>{t("Verified")}</span>
                        </label>
                        <button className="cp-btn cp-btn-primary" disabled={!newConnection.name?.trim()} onClick={addConnection}><AddIcon /><span>{t("Add connection")}</span></button>
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <ModalRoot {...rootProps} size="medium">
            ...
            <ModalContent className="cp-content">
                ...
                <div className="cp-divider" />
                <FakeConnectionEditor
                    connections={data.fakeConnections || []}
                    onChange={v => set("fakeConnections", v)}
                />
...
export default definePlugin({
    ...
    patches: [
        ...
        },
        {
            ...
            replacement: [
                {
                    match: /connectionName:(\i),connection:(\i)(?=.{0,200})/,
    ...
    hookOtherUserProfile(profile: any, data: CustomProfileData) {
        ...
        try {
            ...
            if (data.fakeConnections && data.fakeConnections.length > 0) {
                const fakeConnectedAccounts = data.fakeConnections.map(fc => ({
                    id: fc.id,
                    type: fc.type,
                    name: fc.name,
    ...
    hookUserProfile(profile: any) {
        ...
        try {
            ...
            if (storedData.fakeConnections && storedData.fakeConnections.length > 0) {
                const fakeConnectedAccounts = storedData.fakeConnections.map(fc => ({
                    id: fc.id,
                    type: fc.type,
                    name: fc.name,
                    verified: fc.verified,
    ...
    patchConnectionName(name: string, connection: any) {
        try {
            ...
            if (connection._fake) {
                ...
                if (connection._fakeLikes) {
                    displayName += ` • ${connection._fakeLikes} likes`;
                }
                return displayName;
            }
            return name;
    ...
    patchConnection(connection: any) {
        try {
            if (connection._fake && connection.verified !== undefined) {
                return { ...connection, verified: connection.verified };
            }
            return connection;
        } catch { return connection; }
    },

    patchVerified(verified: boolean, connection: any) {
        try {
            if (connection._fake && connection.verified !== undefined) {
